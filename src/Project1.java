...
public class Project1 {
	...
	public static String srt_simulation(Process[] p) {
		...
		// System.out.printf("time %dms: Process %s arrived and will preempt %s
		// %s\n", t, p.process_id, ss,
		// queueToString(q));
		...
		// System.out.printf("time %dms: Process %s arrived and will preempt %s
		// %s %d\n", t, p.process_id,
		// ss, queueToString(q), processes[y].preemptions);
		...
		// // prempt
		// } else {
		// System.out.printf("time %dms: Process %s arrived and added to ready
		// queue %s\n", t,
		// p.process_id, queueToString(q));
		// }
		//
		// // System.out.printf("time %dms: Process %s arrived and
		// // added to ready queue %s Shorted index at :%d\n", t,
		// // p.process_id, getShortestProcessIndex(processes);
		//
		...
		// if (hasout) {
		// for (Process p : added) {
		// System.out.printf("time %dms: Process %s completed I/O; added to
		// ready queue %s\n", time,
		// p.process_id, queueToString(q));
		// }
		// hasout = false;
		...
		//
		// System.out.printf("time %dms: Process %s started using the CPU with
		// %dms remaining %s\n", t,
		// run.process_id, run.burst_current, queueToString(q));
		//
		...
		// System.out.printf("time %dms: Process %s completed a CPU burst; %d
		// burst%s to go %s\n", t,
		// run.process_id, run.number_bursts, run.number_bursts > 1 ? "s" : "",
		// queueToString(copy));
		// System.out.printf(
		// "time %dms: Process %s switching out of CPU; will block on I/O until
		// time %dms %s\n",
		...
		// run.process_id, queueToString(q));
		...
		// q.remove(p);
		// System.out.printf("time %dms: Process %s completed I/O and will
		// preempt %s %s\n", time,
		...
		// ret += String.format("-- average CPU burst time: %.2f ms\n", (double)
		// cpubursttime / (double) total_cpu_bursts);
		// ret += String.format("-- average wait time: %.2f ms\n", (double)
		// waittime / (double) total_cpu_bursts);
		// ret += String.format("-- average turnaround time: %.2f ms\n",
		...
		while (!queue.isEmpty() || !process_in_io.isEmpty() || !initial_copy.isEmpty() || !simulator.idle()) {
			...
			while (initial_copy.size() != 0) {
				if (global_counter == initial_copy.get(0).initial_arrival_time) {
					...
					if (simulator.getCurrentProcess() != null
						...
						// System.out.print("Process running burst: " +
						// simulator.getCurrentProcess().burst_current + " New
						// burst: " + ps.burst_current);
			...
			if (simulator.idle()) {
				...
				if (simulator.getCurrentProcess().burst_current > 0) { // process
					...
					// System.out.print("time " + global_counter + "Sorted Queue
					// " + queueToString(queue));
					if (ps.burst_current == ps.cpu_burst_time) {
						System.out.print("time " + global_counter + "ms: Process " + ps.process_id
								+ " started using the CPU " + queueToString(queue));
